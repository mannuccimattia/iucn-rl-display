    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-main text-main-contrast overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div
                    class="mb-8 pb-4 flex flex-col gap-y-5 sm:flex-row sm:justify-between items-start items-top border-b border-main-emphasis">
                    <div class="flex items-top">
                        <x-link :href="route('assessments.show', [
                            'type' => 'systems',
                            'code' => $assessment['systems'][0]['code'],
                        ...
                        <div>
                            ...
                            <p class="opacity-60 text-sm">Trend popolazione:
                                {{ $assessment['population_trend']['description']['en'] }}</p>
                        </div>
                    </div>
                            ...
                            <x-card class="p-6 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                                ...
                                @foreach ($actions['actions'] as $action)
                                    <div class="flex flex-col opacity-50 text-sm">
                                        <span>
                                            <i
                                                class="me-1 fa-solid {{ $action['value'] === 'Yes' ? 'fa-check text-green-500' : 'fa-xmark text-red-500' }}"></i>
                                            {{ $action['name'] }}
                                        </span>
                                    </div>
                                @endforeach
                    ...
                    <div class="flex flex-col gap-y-3">
                        @foreach ($assessment['documentation'] as $key => $value)
                            <x-card class="p-6 flex flex-col md:flex-row md:justify-between md:items-top gap-2">
                                <h4 class="w-[200px] text-sm uppercase font-bold">{{ __($key) }}</h4>
                                <p class="w-full text-sm opacity-70">{!! $value ?? 'N/D' !!}</p>
                            </x-card>
                        @endforeach
                    </div>
                </div>

                <div class="mb-8 pb-4 flex flex-col gap-y-2">
                    ...
                    <div class="flex flex-col md:flex-row w-full md:w-auto gap-5">
                        @foreach ($assessment['systems'] as $system)
                            <x-link :href="route('assessments.index', [
                                ...
                                <x-card class="p-6 uppercase text-sm">{{ __($system['description']['en']) }}</x-card>
                            </x-link>
                        @endforeach
                    </div>
                </div>
